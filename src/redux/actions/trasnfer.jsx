...
export const startTransferTo = (amount, accountNumber, senderUserId) => {
  return (dispatch) => {
    ...
    axios
      .post("http://localhost:4001/api/v1/transfers", {
        amount,
        accountNumber,
        senderUserId,
      },
...
export const startGetHistoryTransfer = (id) => {
  return (dispatch) => {
    dispatch(setIsLoading(true));
    axios
      .get(`http://localhost:4001/api/v1/users/${id}/history`, getConfig())
      .then((resp) => {
        console.log(resp);
...
export const startConsign = (data) => {
  return ( dispatch, getState ) => {
    ...
    const { auth } = getState()
    console.log(auth)
    axios.post('http://localhost:4001/api/v1/transfers/upload', {
      amount: data.amount,
      userId: auth.id
    }, getConfig())
    .then(resp => {
...
export const startDeleteAccount = (data) => {
  return ( dispatch, getState ) => {
    ...
    const { auth } = getState()
    console.log(auth)
    axios.patch(`http://localhost:4001/api/v1/users/close/account/${auth.id}`, {
